...
public class SftpHelper extends FtpHelper {
	...
	public void loginFtpServer(String host, String username, String password, int port, int timeout,
		...
		try {
			...
			// 如果服务器连接不上，则抛出异常
			if (session == null) {
				throw DataXException.asDataXException(JXReaderErrorCode.FAIL_LOGIN,
	...
	public HashSet<String> getListFiles(String directoryPath,String pathMather,int parentLevel, int maxTraversalLevel) {
		if(parentLevel < maxTraversalLevel){
			...
			if (isDirExist(directoryPath)) {
                LOG.info( String.format("目录:[%s]", directoryPath));
				// path是目录
                ...
                LOG.info( String.format("链接文件:[%s]", directoryPath));
				//path是链接文件
			...
			}else if (!directoryPath.contains("*") && !directoryPath.contains("?")&&isFileExist(directoryPath)) {
                LOG.info( String.format("*号正则:[%s]", directoryPath));
			    // path指向具体文件
			...
			try {
				...
				Vector vector = channelSftp.ls(parentPath);
