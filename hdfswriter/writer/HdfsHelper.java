...
public  class HdfsHelper {
    ...
    public void getFileSystem(String defaultFS, Configuration taskConfig){
        ...
        if (null != hadoopSiteParams) {
            ...
            for (String each : paramKeys) {
                ...
                if(each.equals("krb5file")){
                	this.kerberosKrb5ConfPath=hadoopSiteParamsAsJsonObject.getString(each);
                }
            }
        }
        ...
        this.kerberosAuthentication(this.kerberosPrincipal,this.kerberosKrb5ConfPath,this.kerberosKeytabFilePath);
    ...
    private void kerberosAuthentication(String kerberosPrincipal,String krb5ConfPath, String kerberosKeytabFilePath){
        if(haveKerberos && StringUtils.isNotBlank(this.kerberosPrincipal) && StringUtils.isNotBlank(this.kerberosKrb5ConfPath) && StringUtils.isNotBlank(this.kerberosKeytabFilePath)){
            try {
            	 KerberosAuthUtil.kerberosAuthentication(kerberosPrincipal, kerberosKeytabFilePath, krb5ConfPath, this.hadoopConf);
            } catch (Exception e) {
        		...
        		for (Iterator it1=tmpFiles.iterator(),it2=endFiles.iterator();it1.hasNext()&&it2.hasNext();){
            		...
            		for (String srcFile : tempFileList) {
            			...
            			String dstFile = String.format("[%s][%s]", endDir,targetFileName); 			
            			 LOG.info(String.format("start rename file [%s] to file [%s].", srcFile,dstFile));
            			 Path dstFilePath = new Path(targetFileName);
        ...
        if (0 != recordLength) {
            ...
            for (int i = 0; i < recordLength; i++) {
                ...
                if (null != column.getRawData()) {
                    ...
                    //columnsConfiguration为空的时候默认以字符串进行写入
                    if(null==columnsConfiguration||columnsConfiguration.size()==0){
